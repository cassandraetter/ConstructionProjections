...
```{r}
weights <- read_excel("~/ConstructionProjections/Data/weights.xlsx")


BS_2050 <- residential %>%
  select(Country = `Country Name`, iso3c, year, population,average_household_size,num_households, gdp_per_capita,avg_m2_capita, residential_building_stock_m2) %>%
  ...
  left_join(read_excel("~/ConstructionProjections/Data/weights.xlsx") %>% as.data.table %>%
              select(Region, residential), by = "Region") %>%
